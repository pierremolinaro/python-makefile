...
\usepackage{tikz}              %--- Pour dessiner des figures
\usepackage{pgfplots}
\usetikzlibrary{decorations}
\usetikzlibrary{decorations.pathmorphing}
\usetikzlibrary{arrows}        %--- Bibliothèques de tracés de flèches
\usepgflibrary{shapes.arrows}    %--- Flèches dessinées par contour 
\usetikzlibrary{automata}      %--- Bibliothèque de tracés d'automates
\usetikzlibrary{positioning}    %--- Placements de noeuds relativement les uns aux autres
\usetikzlibrary{shadows.blur}    %--- Pour l'ombrage flouté des formes fermées
\usepackage{tikzpfeile}        %--- Pour des tracés de flèches complexes
\usepackage{schemabloc}        %--- Pour construire des fonctions de transfert
\usepackage{tikz-cd}        %--- Pour construire des diagrammes fléchés simples
\usepackage{tikz-timing}      %--- Pour construire des chronogrammes
\usetikztiminglibrary{counters}
\usepackage{tkz-base}        %--- Pour le tracé de système d'axes, de grilles
\usepackage{tkz-fct}        %--- Pour le tracé de fonctions (x,y)
\usepackage{tkz-euclide}      %--- Macros de base de construction de formes géométriques 2D
\usetkzobj{polygons}        %--- Macros de construction des polygones de tkz-euclide
\usetkzobj{circles}          %--- Macros de construction des cercles de tkz-euclide
...
La classe \texttt{makefile} permet de construire facilement des scripts Python qui ont toutes les facilités des fichiers \emph{makefile} exécutés par l'utilitaire \texttt{make}. Les avantages principaux de la classe \texttt{makefile} sont :
\begin{itemize}
...
~\\On récapitule la construction des règles relatives à la compilation des sources C :
\begin{lstlisting}[language=py]
sourceList = ["main.c", "myRoutine.c"]
objectList = []
for source in sourceList:
...
~\\Nous allons maintenant définir les buts (« \emph{goals} »). Il faut en définir au moins un. Ici, nous allons en définir deux :
\begin{itemize}
...
~\\Les règles de compilation des sources C deviennent donc :
\begin{lstlisting}[language=py]
sourceList = ["main.c", "myRoutine.c"]
objectList = []
for source in sourceList:

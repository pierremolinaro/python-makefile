...
{\bf Nouveau dans la version 2.3.}\label{affichagePourcentage} Le pourcentage qui est affiché représente le travail qui sera effectué lorsque la commande citée sur la même ligne sera réalisée. Aussi, la dernière commande est toujours précédée de « \texttt{[100\%]} ». Si on ne veut pas afficher cette information, il suffit d'appeller la méthode \texttt{doNotShowProgressString} de la classe \texttt{Make} avant d'appeler \texttt{runGoal} :
